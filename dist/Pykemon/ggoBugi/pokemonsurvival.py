def pokemonsurvival():
    ...
    # 이미지 로드 (플레이어, 총알, 적)
    path = "Pykemon\ggoBugi\pokesurv_image/"
    player_up = pygame.image.load(path+'player_up.png')
    player_down = pygame.image.load(path+'player_down.png')
    player_left = pygame.image.load(path+'player_left.png')
    player_right = pygame.image.load(path+'player_right.png')
    player_upleft = pygame.image.load(path+'player_upleft.png')
    player_upright = pygame.image.load(path+'player_upright.png')
    player_downleft = pygame.image.load(path+'player_downleft.png')
    player_downright = pygame.image.load(path+'player_downright.png')

    bullet_image = pygame.image.load(path+'bullet.png')
    ...
    while running:
        ...
        # 체력 비율에 따라 색상 결정
        player_health_bar_color = GREEN if player_health_ratio > 0.5 else RED

        # 체력 바 배경 (회색)
        pygame.draw.rect(screen, WHITE, (10, 70, player_health_bar_width, player_health_bar_height))
        # 체력 바
        pygame.draw.rect(screen, player_health_bar_color, (10, 70, player_health_bar_width * player_health_ratio, player_health_bar_height))
